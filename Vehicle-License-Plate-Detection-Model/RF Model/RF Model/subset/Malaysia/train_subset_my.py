...
###############################################################################################################################################
    ...
    joblib.dump(rf_clf, "MRF_compressed_subset.joblib", compress=3)  # compression is ON!
    print(f"Compressed Random Forest: {np.round(os.path.getsize('MRF_compressed_subset.joblib') / 1024 / 1024, 2) } MB")
    ...
    for i,ch in enumerate(crop_characters):
        ...
        probability=rf_clf.predict_proba(l)
        a_list = Categories[rf_clf.predict(l)[0]]
